...
# dataRetrieval and area normalized flows (12 pts)

[**Repo here**](https://github.com/tpcovino/06_data_retrieval_lab){target="_blank"}

## Intro

In this lab we are going to use dataRetrieval and other packages (e.g., tidyverse, patchwork, leaflet) to download and analyze flow data. You are going to download streamflow data for gauges in Park County Montana. You will start by exploring how many gauges there are in the county and mapping them to see where they are. Next you will download the data from the gauges and do some exploratory data analysis (EDA). You will then do some data analysis on a subset of gauges that meet certain criteria. 

Learning objectives: At the end of this lab students will <br>
1. Be able to download data using dataRetrieval <br>
2. Understand volumetric and area normalized flow and their utility <br>
3. Assess flow generation from different landscapes and propose an explanation for any differences. 
...
Use dataRetrieval to see how many USGS streamflow gauges there are in Park County that have data from 10/1/1990 to the end of the previous water year (change dates below).
```{r}
...
The section below is the new function that will replace "whatNWISsites"
```{r}
#out_sf <- 
...
# The code below is for the new function of dataRetrieval (i.e., read_waterdata_daily)

#sites_v2 <- paste("USGS-", stream_gauges$site_no, sep = "") 

# flows_v2 <- read_waterdata_daily(
#   monitoring_location_id = sites_v2,
#   parameter_code = param_cd
# )

# OR

# mt_test <- read_waterdata_monitoring_location(state_name = "Montana") %>% 
  # filter(site_type == "Stream")

# And then thin for sites in Park county. 

#write_csv(flows, "flows.csv")
#flows <- read_csv("flows.csv")
```

Make a facet wrap to look at the volumetric flows for each gauge. Facet by "site_no". What do you observe? 
```{r}
...
Make another facet wrap of volumetric flow at those 4 gauges. Again, filter by "site_no". What differences in volumetric flow do you observe among the gauges? 
```{r}
...
Now that you have flow at each site in mm/day, make a facet wrap of flow for each gauge in mm/day. This area normalized flow gives us a sense of how much flow a watershed produces per unit area. It also allows us to compare across watersheds of different sizes. Obviously a larger watershed will produce more volumetric flow but will it produce more flow per unit area? Think about it like an hourly wage. You might make more at work because you work more hours, but your friend might make a higher hourly wage. So volumetric is like how much you made this pay period, and area normalized is like how much you made divided by the number of hours you worked. So wet watersheds that produce a lot of flow per unit area, have a high hourly wage, so to speak. 

```{r}

```

Compute and plot total annual flow (in mm/yr) for each gauge. You will need group_by(site_no, station_nm, year), and reframe. We did this is the first few weeks. Remember to add in na.rm = TRUE in case there are any NA values.  
...
Use your plots of volumetric and area normalized flow mm/year to comment on:\

1. Across the 4 gauges - compare both the volumetric and normalized flows. What do you observe? Are volumetric and area normalized relationships between the gauges the same or different? (2 pts)

2. Does one gauge produce consistently less flow year to year? Does one gauge produce consistently more flow year to year? (2 pts)\

3. Which gauges show similar patterns in annual water yield? Provide a plausible explanation for why that might be. (2 pts)\

4. For any gauge(s) that seem different than the others in terms of area normalized flow, provide an explanation from a climate perspective for why that might be. (2 pts) 
